...
async function seed() {
  ...
  await pool.execute('TRUNCATE TABLE notification');
  await pool.execute('TRUNCATE TABLE document');
  await pool.execute('TRUNCATE TABLE historique_statut');
  await pool.execute('TRUNCATE TABLE requete_note');
  await pool.execute('TRUNCATE TABLE requete_correction_nom');
  await pool.execute('TRUNCATE TABLE requete_attestation');
  await pool.execute('TRUNCATE TABLE requete');
  await pool.execute('TRUNCATE TABLE etudiant');
  await pool.execute('TRUNCATE TABLE personnel');
  ...
  for (let i = 0; i < 9; i++) {
    await pool.execute(
      ...
      [userIds[i], `IUT2024${String(i + 1).padStart(3, '0')}`, filieres[i], niveaux[i]]
    );
  }
  ...
  for (const p of personnels) {
    await pool.execute(
      'INSERT INTO personnel (user_id, poste, service, departement) VALUES (?, ?, ?, ?)',
      [userIds[p.idx], p.poste, p.service, p.departement]
    );
  }
  ...
  const [etudiants]: any = await pool.execute('SELECT id, user_id FROM etudiant ORDER BY id');
  const eIds = etudiants.map((e: any) => e.id);
  ...
  const requeteIds: number[] = [];

  for (const r of requetes) {
    const [result]: any = await pool.execute(
      'INSERT INTO requete (etudiant_id, type, statut, priorite) VALUES (?, ?, ?, ?)',
      [r.etudiant_id, r.type, r.statut, r.priorite]
    );
  ...
  for (let i = 0; i < 4; i++) {
    await pool.execute(
      ...
      [requeteIds[i], 'attestation_scolarite', '2023-2024', 'Demande pour dossier de bourse']
    );
  }
  ...
  for (let i = 4; i < 8; i++) {
    await pool.execute(
      ...
      [requeteIds[i], 'DJONKOUN', 'DJONKOUNG', 'Erreur orthographique sur acte de naissance']
    );
  }
  ...
  for (let i = 8; i < 12; i++) {
    await pool.execute(
      ...
      [requeteIds[i], 'INF301', 8.5, 12.0, 'La note ne correspond pas à ma copie corrigée rendue en cours lors de la session de janvier 2024', userIds[15]]
    );
  }
  ...
  for (let i = 0; i < requeteIds.length; i++) {
    ...
    await pool.execute(
      ...
      [requeteIds[i], null, 'EN_ATTENTE', userIds[i % 9], 'Requête soumise par l\'étudiant']
    );

    if (['EN_COURS', 'ATTENTE_INFO', 'VALIDEE', 'EN_EXECUTION', 'REJETEE', 'CLOTUREE'].includes(requetes[i].statut)) {
      await pool.execute(
        'INSERT INTO historique_statut (requete_id, ancien_statut, nouveau_statut, change_par, commentaire) VALUES (?, ?, ?, ?, ?)',
        [requeteIds[i], 'EN_ATTENTE', 'EN_COURS', secretariatId, 'Dossier réceptionné par le secrétariat']
      );
    }
    ...
    if (['VALIDEE', 'EN_EXECUTION', 'CLOTUREE'].includes(requetes[i].statut)) {
      await pool.execute(
        'INSERT INTO historique_statut (requete_id, ancien_statut, nouveau_statut, change_par, commentaire) VALUES (?, ?, ?, ?, ?)',
        [requeteIds[i], 'EN_COURS', 'VALIDEE', directeurId, 'Dossier validé par la direction']
      );
    }
    ...
    if (requetes[i].statut === 'EN_EXECUTION') {
      await pool.execute(
        'INSERT INTO historique_statut (requete_id, ancien_statut, nouveau_statut, change_par, commentaire) VALUES (?, ?, ?, ?, ?)',
        [requeteIds[i], 'VALIDEE', 'EN_EXECUTION', userIds[17], 'Modification en cours par la cellule informatique']
      );
    }
    ...
    if (requetes[i].statut === 'CLOTUREE') {
      await pool.execute(
        'INSERT INTO historique_statut (requete_id, ancien_statut, nouveau_statut, change_par, commentaire) VALUES (?, ?, ?, ?, ?)',
        [requeteIds[i], 'EN_EXECUTION', 'CLOTUREE', scolariteId, 'Dossier clôturé avec succès']
      );
    }
    ...
    if (requetes[i].statut === 'REJETEE') {
      await pool.execute(
        'INSERT INTO historique_statut (requete_id, ancien_statut, nouveau_statut, change_par, commentaire) VALUES (?, ?, ?, ?, ?)',
        [requeteIds[i], 'EN_COURS', 'REJETEE', directeurId, 'Dossier incomplet — pièces insuffisantes']
      );
    }
    ...
    if (requetes[i].statut === 'ATTENTE_INFO') {
      await pool.execute(
        'INSERT INTO historique_statut (requete_id, ancien_statut, nouveau_statut, change_par, commentaire) VALUES (?, ?, ?, ?, ?)',
        [requeteIds[i], 'EN_COURS', 'ATTENTE_INFO', secretariatId, 'Pièce manquante : Quitus requis']
      );
    }
  }
  ...
  for (let i = 0; i < requeteIds.length; i++) {
    await pool.execute(
      ...
      [userIds[requetes[i].etudiant_id - eIds[0]], requeteIds[i], `Votre requête #${requeteIds[i]} a été soumise avec succès`]
    );
  }
