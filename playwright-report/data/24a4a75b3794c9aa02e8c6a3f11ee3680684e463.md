...
- Name: workflow1-effet-academique.spec.ts >> Workflow 1 — Effet Académique (end-to-end) >> Step 5: Directeur validates the request
- Location: e2e\workflow1-effet-academique.spec.ts:72:7
...
Call log:
  - waiting for locator('tr').filter({ hasText: 'EN COURS' }).first().getByRole('link', { name: /traiter/i })
